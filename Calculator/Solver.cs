...
namespace Calculator
{
    public class Solver
    {
        ...
        {
            lhs.Sort((x, y) => x.Pronumeral.CompareTo(y.Pronumeral));
            //LHS Plus (Should be only pronumerals)
            for (int i = 1; i < lhs.Count; i++)
            {
                if (lhs[i].Pronumeral.Equals(lhs[i - 1].Pronumeral))
                {
            ...
            {
                if (rhs[i].Modifier == Modifier.NONE && rhs[i].Pronumeral.Equals("") && rhs[i - 1].Pronumeral.Equals(""))
                {
        ...
        private void MulDivideModulus(List<Term> side)
        {
            for (int i = 1; i < side.Count; i++)
            {
                ...
                if (side[i].Modifier == Modifier.MUL)
                {
                    ...
                    {
                        side[i].Pronumeral = side[i - 1].Pronumeral;
                    }
